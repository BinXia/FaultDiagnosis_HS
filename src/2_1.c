...
{	
	...
	// 預處理數據
	DNode head = pre_data[2]->next;	// 獲取數據鏈表
	...
	globalK = fabs((start_point->num - data[2])/((double)(time_period)/PreSIZE));
	double localK;						// 導葉卡頓與突變﹣參考數值
	localK = fabs((pre_point->num - data[2])/(double)(1.0/PreSIZE));
